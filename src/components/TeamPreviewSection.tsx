                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {team.map((member, i) => (
                        <div key={i} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden text-center p-6 hover:shadow-md transition group">
                            <div className="w-24 h-24 mx-auto rounded-full bg-gray-50 flex items-center justify-center mb-6 border border-gray-100 group-hover:border-bislyRed/20 transition-colors">
                                <svg className="w-12 h-12 text-gray-300 group-hover:text-bislyRed/40 transition-colors" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" />
                                </svg>
                            </div>
                            <h3 className="text-lg font-bold text-[#1A1A1A] mb-1">{member.name}</h3>
                            <p className="text-bislyRed font-semibold text-xs uppercase tracking-wider mb-4">{member.role}</p>
